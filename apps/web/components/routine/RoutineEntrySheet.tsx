...
const CATEGORIES: CategoryConfig[] = [
  { key: "FEEDING",    label: "Alimentação", Icon: Utensils, color: "var(--color-accent)", fields: ["notes"]                    },
  { key: "SLEEP",      label: "Sono",        Icon: Moon,     color: "var(--color-accent)", fields: ["duration", "notes"]        },
  { key: "CRISIS",     label: "Crise",       Icon: Zap,      color: "var(--color-accent)", fields: ["duration", "severity", "notes"] },
  { key: "SCHOOL",     label: "Escola",      Icon: BookOpen, color: "var(--color-accent)", fields: ["notes"]                    },
  { key: "MEDICATION", label: "Medicação",   Icon: Pill,     color: "var(--color-accent)", fields: ["quantity", "notes"]        },
]
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-4 pt-2">
          <h2 className="text-lg font-semibold text-[var(--color-text)]">Registrar</h2>
          <button
            onClick={handleClose}
            ...
            <button
              ...
              className={cn(
                ...
                category === c.key
                  ...
                  : "bg-[var(--color-bg)] text-[var(--color-muted)] hover:bg-[var(--color-border)]"
              )}
            >
        ...
        <div className="flex flex-col gap-5 px-5 pb-6">
          ...
          {cfg.fields.includes("duration") && (
            <div className="flex flex-col gap-2">
              <span className="text-xs font-medium text-[var(--color-muted)]">Duração</span>
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setDuration((v) => Math.max(5, v - 15))}
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-bg)] text-[var(--color-text)] active:bg-[var(--color-border)]"
                  aria-label="Diminuir"
                >
                  <Minus size={16} aria-hidden />
                </button>
                <span className="flex-1 text-center text-xl font-semibold text-[var(--color-text)]">
                  {durationMin >= 60
          ...
          {cfg.fields.includes("severity") && (
            <div className="flex flex-col gap-2">
              <span className="text-xs font-medium text-[var(--color-muted)]">Intensidade da crise</span>
              ...
              <div className="flex justify-between">
                <span className="text-[10px] text-[var(--color-muted-2)]">Leve</span>
                <span className="text-[10px] text-[var(--color-muted-2)]">Severa</span>
              </div>
            </div>
          )}
          ...
          {cfg.fields.includes("quantity") && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs font-medium text-[var(--color-muted)]">Dosagem / Medicamento</span>
              <input
                type="text"
                value={quantity}
          ...
          {/* Notes */}
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[var(--color-muted)]">Observações (opcional)</span>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="O que aconteceu?"
              rows={3}
              className="resize-none rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-4 py-3 text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-accent)] focus:bg-[var(--color-surface)] transition-colors placeholder:text-[var(--color-muted-3)]"
            />
          </label>
