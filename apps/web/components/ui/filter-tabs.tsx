...
export function FilterTabs({ tabs, active, onChange }: FilterTabsProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex items-center gap-1 rounded-full bg-white p-1 shadow-card"
      role="tablist"
    >
      {/* Sliding pill — absolutely positioned, driven by GSAP */}
      <span
        ref={pillRef}
        className="pointer-events-none absolute top-1 bottom-1 rounded-full bg-[#111111] shadow-sm"
        aria-hidden
      />
      ...
      {tabs.map((t) => (
        <button
          ...
          className={cn(
            ...
            active === t.key ? "text-white" : "text-[#AAAAAA] hover:text-[#666666]"
          )}
        >
